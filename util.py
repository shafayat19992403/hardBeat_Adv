...
def get_loader(dataset, train, batch_size, ratio=0.01):
    if dataset == 'cifar10':
        if train:
        ...
        transform = T.Compose([T.ToTensor(),
                               T.Normalize(_mean[dataset], _std[dataset])])
        dataset = SVHN(root='./data/svhn', split=split, transform=transform, download=True)
    elif dataset == 'gtsrb':
        split = 'train' if train else 'test'
        ...
        transform = T.Compose([T.Resize(size),
                               ...
                               T.ToTensor(),
                               T.Normalize(_mean[dataset], _std[dataset])])
        dataset = GTSRB(root='./data', split=split, transform=transform, download=True)
...
def decision_function(model, inputs, normalize, target):
    ...
    pred = model(normalize(inputs).to(device)).max(dim=1)[1]
    #print(pred, target)
    compare = torch.eq(pred, target)
    return compare

# def decision_function(model, inputs, normalize, target):
#     inputs = torch.clamp(inputs, 0, 1)  # Ensure inputs are within valid range
#     device = inputs.device  # Use the same device as inputs for consistency
#     normalized_inputs = normalize(inputs).to(device)
#     outputs = model(normalized_inputs)
#     pred = torch.argmax(outputs, dim=1)  # Convert logits to class predictions
#     compare = torch.eq(pred, target)  # Ensure this comparison is valid
#     return compare
